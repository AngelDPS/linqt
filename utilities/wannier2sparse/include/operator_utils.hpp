...
namespace oputil
{
  ...
  string AVAIL_OPS[] = {	
    ...
    "SX", "SY", "SZ", "TX", "TY", "TZ", "PS+X",
    "PS-X", "PS+Y", "PS-Y", "PS+Z", "PS-Z"
  }; 
  ...
  /**
   * Check if the string labeling an operator belongs to any of the defined spin projector operators. 
   *
   * @param[out] is_op  true if it belongs false if not
   * @param[in]  op string containing the operator label.
   */
  inline
  bool is_spin_projector(string op)
  {
    return is_operator_in(op, 18, 24);
  };
  ...
  /**
   * Extract the orientation (?) of the spin fron the operator label. This function does not check if the operator belong to a spin projection operator. 
   *
   * @param[out] orientation  The orientation of the spin which can be = -1,+11;
   * @param[in]  op string containing the operator label.
   */
  inline
  char spin_orientation(string op)
  {
    return op[2];
  };
